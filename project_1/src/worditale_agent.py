...
def plan_story(state: StoryState) -> dict:
    ...
    return {"story_plan": plan, "retry_count": 0, "issues": [], "illust_prompts": None}
...
def write_pages_toddler(state: StoryState) -> dict:
    ...
    if _llm_provider():
        ...
        pages = [{"page": 1, "text": "토토가 아침에 눈을 떴어요. 까꿍!"}]
...
def gen_illust_prompt(payload: dict) -> dict:
    ...
    if _llm_provider():
        prompt = _call_llm(
            f"유아 동화책 삽화 지시문을 영어 한 문장으로 써줘. 스타일: 파스텔톤 수채화, 밝고 포근하게. "
            f"테마: {theme}. 장면: {page['text']}",
            max_tokens=200,
        )
    else:  # mock
        prompt = (
            f"{theme} 배경, 아기 토끼 토토 — '{page['text']}' 장면을 "
            f"파스텔톤 수채화로, 밝고 포근한 유아 동화책 삽화 스타일"
        )
...
def fanout_illustrations(state: StoryState) -> list[Send]:
    ...
    return [Send("gen_illust_prompt", {"page": p, "theme": theme}) for p in state["pages"]]
